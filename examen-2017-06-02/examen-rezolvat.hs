...
areX  X                       = True
areX (Intreg _)               = False
areX (e1 :+: e2)              = areX e1 || areX e2
areX (e1 :-: e2)              = areX e1 || areX e2
areX (e1 :*: e2)              = areX e1 || areX e2
areX (DacaMaiMic e1 e2 e3 e4) = all areX [e1, e2, e3, e4]
...
calculeaza X i = i
calculeaza (Intreg i) _ = i
calculeaza (e1 :+: e2) i = i1 + i2
  where
    i1 = calculeaza e1 i
    i2 = calculeaza e2 i
calculeaza (e1 :-: e2) i = i1 - i2
...
calculeaza (e1 :*: e2) i = i1 * i2
  where
    i1 = calculeaza e1 i
    i2 = calculeaza e2 i
calculeaza (DacaMaiMic e1 e2 e3 e4) i = if i1 < i2 then i3 else i4
  where
    i1 = calculeaza e1 i
    i2 = calculeaza e2 i
    i3 = calculeaza e3 i
...
protejeaza e
  | areX e = DacaMaiMic X (Intreg 0) (Intreg 0) e'
  | otherwise = e'
  where e' = protejeaza' e

protejeaza' :: Expresie -> Expresie
protejeaza' (Intreg i)  = Intreg (if i < 0 then 0 else i)
protejeaza' X           = X
protejeaza' (e1 :+: e2) = e1' :+: e2'
  where
    e1' = protejeaza' e1
    e2' = protejeaza' e2
protejeaza' (e1 :-: e2) = DacaMaiMic e1' e2' (Intreg 0) (e1' :-: e2')
  where
    e1' = protejeaza' e1
    e2' = protejeaza' e2
protejeaza' (e1 :*: e2) = e1' :*: e2'
  where
    e1' = protejeaza' e1
    e2' = protejeaza' e2
protejeaza' (DacaMaiMic e1 e2 e3 e4) = DacaMaiMic e1' e2' e3' e4'
  where
    e1' = protejeaza' e1
    e2' = protejeaza' e2
    e3' = protejeaza' e3
...
evalAExp :: AExp -> M Int
evalAExp (Lit i) = return i
evalAExp (Var x) = lookUp x
evalAExp (Plus a1 a2) = do
  i1 <- evalAExp a1
  i2 <- evalAExp a2
  return $ i1 + i2
evalAExp (Minus a1 a2) = do
  i1 <- evalAExp a1
  i2 <- evalAExp a2
  return $ i1 - i2
...
evalStmt (s1 :# s2) = do
  evalStmt s1
  evalStmt s2
evalStmt (If a s1 s2) = do
  i <- evalAExp a
  if i < 0
    then evalStmt s1
    else evalStmt s2
evalStmt (Decl x) = update x 0
evalStmt (x := a) = do
  i <- evalAExp a
  update x i

update :: Id -> Int -> M ()
update x i = modify $ ((x,i) :) . filter (\ (y,_) -> y /= x)

run :: Stmt -> Mem
run s = snd $ runState (evalStmt s) []
